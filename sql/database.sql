...
ALTER TABLE public.user_groups ADD COLUMN created_by INTEGER  REFERENCES auth_user(id);
ALTER TABLE public.user_groups ADD COLUMN updated_by INTEGER  REFERENCES auth_user(id);
...
CREATE TABLE roles (
    ...
    icon VARCHAR(255),
    label VARCHAR(255),
    rol_parent_id INTEGER
);

ALTER TABLE roles
    ADD CONSTRAINT fk_roles_parent FOREIGN KEY (rol_parent_id) REFERENCES roles (id);
...
INSERT INTO users (firstname, lastname, type, idem, phone, email) VALUES
